    ...
    BEGIN
        ...
        WHILE v_tname IS NOT NULL LOOP
            ...
            IF NOT v_prod_tables.EXISTS(v_tname) THEN
                -- Таблица есть в Dev, нет в Prod
                v_diff_tables.EXTEND;
                v_diff_tables(v_diff_tables.LAST) := v_tname;
            ELSE
                -- Таблица есть в обеих схемах — сравниваем структуру
                SELECT COUNT(*) INTO v_col_diff
                FROM (
                    -- Столбцы есть в Dev, нет в Prod или разные типы
...
BEGIN
    ...
    -- --------------------------------------------------------
    -- 2.2 Процедуры и функции (по тексту исходного кода)
    -- --------------------------------------------------------
...
BEGIN
    ...
    -- --------------------------------------------------------
    -- 3.1 Таблицы: CREATE для новых, ALTER для изменённых
    -- --------------------------------------------------------
    out('-- ---- ТАБЛИЦЫ ----');
    out('');
    ...
    -- --------------------------------------------------------
    -- 3.2 Процедуры/Функции: CREATE OR REPLACE для изменённых
    -- --------------------------------------------------------
    ...
    -- --------------------------------------------------------
    -- 3.4 Индексы: CREATE для новых / отличающихся
    -- --------------------------------------------------------
    out('-- ---- ИНДЕКСЫ ----');
    out('');
...
-- Задание 1: базовое сравнение таблиц
-- EXEC compare_schemas('DEV_SCHEMA', 'PROD_SCHEMA');

-- Задание 2: расширенное сравнение (таблицы + процедуры + функции + индексы + пакеты)
