...
Симулятор атак на переполнение буфера.

Запускать ПОСЛЕ server.py.

Атаки:
    1  — Точное заполнение буфера (нормальный ввод)
    2  — Переполнение буфера (данные > 16 байт)
    3  — Попытка затереть канарейку
    4  — Попытка записи shell-кода (NOP-sled + payload)
    5  — Off-by-one (ровно на 1 байт больше буфера)
    6  — Огромный сетевой пакет
    7  — Cyclic pattern (определение смещения до ret-addr)
    8  — Null-byte инъекция
    9  — Integer overflow (отрицательный/нулевой размер)
    10 — ROP-цепочка (имитация Return-Oriented Programming)
